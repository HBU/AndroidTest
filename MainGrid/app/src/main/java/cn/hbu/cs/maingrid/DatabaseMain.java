...
public class DatabaseMain extends AppCompatActivity {
            ...
            try{
                switch (button.getId()) {//根据按钮id，判断点击了那个按钮，进一步执行相关代码
                    case R.id.db_create: {                        //create Database
                        dbHelper = new DBHelper(DatabaseMain.this, "school.db", null, 3);//创建帮助器对象
                        database = dbHelper.getWritableDatabase();//创建数据库对象
                        Toast.makeText(DatabaseMain.this,"数据库已建立",Toast.LENGTH_SHORT).show();
                        database.close();
                        break;
                    }
                    case R.id.db_init: {                        //Init Table
                        Toast.makeText(DatabaseMain.this,"新写入10条数据",Toast.LENGTH_SHORT).show();
                    ...
                    case R.id.db_list: {                        //Show all data
                        Toast.makeText(DatabaseMain.this,"显示全部数据",Toast.LENGTH_SHORT).show();
                        initDbData();//重新初始化RecyclerView
                        initView();
                        break;
                    }
                    case R.id.db_insert:{                        //Insert one line data
                        Toast.makeText(DatabaseMain.this,"插入一条数据",Toast.LENGTH_SHORT).show();
                        database = dbHelper.getWritableDatabase();
                    ...
                    case R.id.db_delete:{                        //Delete the selected data
                        Toast.makeText(DatabaseMain.this,"删除一条数据",Toast.LENGTH_SHORT).show();
                        database = dbHelper.getWritableDatabase();
                        int DeleteId;
                    ...
                    case R.id.db_update:{                        //Update the selected data
                        Toast.makeText(DatabaseMain.this,"更新一条数据",Toast.LENGTH_SHORT).show();
                        database = dbHelper.getWritableDatabase();
                    ...
                    case R.id.db_query:{                        //Query target data
                        Toast.makeText(DatabaseMain.this,"查询当前学号学生",Toast.LENGTH_SHORT).show();
    ...
    private void initDbData() {// 初始化RecyclerView数据，把数据库内容写入
        ...
        mAdapter = new DB_Adaptor(getStudentList());
    }
    private void initLineData() {// 初始化RecyclerView数据，把数据库一条查询结果写入
        mLayoutManager = new LinearLayoutManager(this, LinearLayoutManager.VERTICAL, false);
        mAdapter = new DB_Adaptor(getStudentLine());
    }
    ...
    public ArrayList<Student> getStudentList() {//获取全部数据
        dbHelper = new DBHelper(DatabaseMain.this, "school.db", null, 3);//This is the key. 2017.6.21 4:10 AM
        database = dbHelper.getWritableDatabase();
    ...
    public ArrayList<Student>  getStudentLine() {// 处理查询结果，类似获取全部数据
        dbHelper = new DBHelper(DatabaseMain.this, "school.db", null, 3);
        database = dbHelper.getWritableDatabase();
